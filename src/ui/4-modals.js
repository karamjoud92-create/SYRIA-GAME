...
function cycleSvg(id){
  ...
  const labels = nodes.map((txt, i) => { const [x, y] = pos[i]; return `<g><rect x="${x - 62}" y="${y - 16}" width="124" height="32" rx="16" fill="var(--panel2)" stroke="${col}" stroke-width="1.5"/><text x="${x}" y="${y + 5}" text-anchor="middle" class="cyc">${esc(txt)}</text></g>`; }).join('');
  return `<svg viewBox="-30 0 400 300" class="cyclesvg" direction="ltr" aria-hidden="true"><defs><marker id="ah" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10z" fill="${col}"/></marker></defs>${arcs}${labels}<text x="${cx}" y="${cyy + 6}" text-anchor="middle" font-size="30">${cy.bad ? '🔻' : '🔺'}</text></svg>`;
}
