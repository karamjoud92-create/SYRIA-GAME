...
function whyChains(before, after){
  ...
  if (Math.abs(w.pct) >= 1){
    ...
    out.push({ icon:'exchange', steps:c, weight:Math.abs(w.pct) * 2 });
  }
  // 2) trust chain
  { const dt = w.trustAfter - w.trustBefore;
    if (Math.abs(dt) >= 0.5){
      ...
      if (parts.length) out.push({ icon:'handshake', weight:Math.abs(dt) * 1.5, steps:[step(parts.map(([k, v]) => `${W['t_' + k]} (${sign(v, 0)})`).join(' + '), 'cause'),
  ...
  { const da = natUnrest(after) - natUnrest(before);
    if (Math.abs(da) >= 0.5){
      ...
      if (parts.length) out.push({ icon:'flame', weight:Math.abs(da) * 1.5, steps:[step(parts.map(([k]) => W[(da > 0 ? 'a_' : 'b_') + k]).join(' + '), 'cause'),
  ...
  { const d = after.treasury - before.treasury, rows = after.last.ledger.syp;
    if (Math.abs(d) >= 3){
      ...
      out.push({ icon:'cash', weight:Math.abs(d) / 3, steps:c });
    } }
  // 5) delayed power
  { const done = after.last.notes.find(n => n[0] === 'gridDone'), dh = w.hrs1 - w.hrs0;
    if (done && dh > 0.1) out.push({ icon:'bolt', weight:4, steps:[step(W.gridArrived + ` (+${done[1]} MW)`, 'cause'), step(fill(W.powerUp, [dh.toFixed(1)]), 'good'), step(W.a_power + ' ↓', 'good')] }); }
...
function cycleSvg(id){
  ...
  return `<svg viewBox="-60 0 460 300" class="cyclesvg" direction="ltr" aria-hidden="true"><defs><marker id="ah" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10z" fill="${col}"/></marker></defs>${arcs}${labels}<text x="${cx}" y="${cyy + 6}" text-anchor="middle" font-size="30">${cy.bad ? 'down' : 'up'}</text></svg>`;
}
...
// ---------- modals ----------
const MODAL_LOCKED = ['crisis', 'fail', 'legacy', 'missionEnd'];
function modal(html, cls = ''){
  const locked = MODAL_LOCKED.some(k => (cls || '').split(/\s+/).includes(k));
  const x = locked ? '' : `<button class="mclose" data-act="close" aria-label="${t('close')}">${ic('close')}</button>`;
  $('#modal').innerHTML = `<div class="scrim" role="dialog" aria-modal="true"><div class="modal ${cls}">${x}${html}</div></div>`;
  const f = $('#modal .btn.primary') || $('#modal button:not(.mclose)') || $('#modal button');
  if (f) f.focus();
}
...
function startScreen(){
  modal(`<div class="row spread"><div class="tut-icon" aria-hidden="true">${ic('olive')}</div><button class="btn" data-act="lang">${ic('globe')} ${t('language')}</button></div>
...
function missionsScreen(){
  modal(`<h2>${ic('target')} ${t('missionsTitle')}</h2>` + Object.keys(MISSIONS).map(id => { const m = MISSION_TXT[id], x = L2(m);
...
function gloss(k){
  ...
  modal(`<div class="tut-icon" aria-hidden="true">${ic(g.icon)}</div><h2>${G.name}</h2><p class="lede">${esc(G.what)}</p><p><b>${t('why')}</b> ${esc(G.why)}</p><p><b>${t('how')}</b> ${esc(G.fix)}</p>
  <button class="btn primary" data-act="close">${t('gotIt')}</button>`);
}
function menu(){
  modal(`<h2 data-menu="1">${t('menu')}</h2><p class="muted" style="margin:0 0 16px">${t('menuSaved')}</p>
  <div class="row"><button class="btn primary" data-act="close">${t('backToGame')}</button><button class="btn" data-act="tut" data-i="0">${t('howToPlay')}</button><button class="btn" data-act="lang">${ic('globe')} ${t('language')}</button></div>
  <div class="row" style="margin-top:10px"><button class="btn" data-act="restart">${t('newGame')}</button></div>`);
}
function saveCodeModal(){
  const code = saveCode();
  try { navigator.clipboard && navigator.clipboard.writeText(code).catch(() => {}); } catch(e){}
  modal(`<h2>${ic('box')} ${t('saveCode')}</h2><p>${t('saveCopied')}</p><textarea class="code" readonly onclick="this.select()">${code}</textarea><button class="btn primary" data-act="close">${t('gotIt')}</button>`);
  const ta = $('#modal textarea'); if (ta){ ta.focus(); ta.select(); }
}
function loadCodeModal(err){
  modal(`<h2>${ic('box')} ${t('loadCode')}</h2>${err ? `<p class="bad">${t(UI.loadErr === 'newer' ? 'badCodeNewer' : UI.loadErr === 'broken' ? 'badCodeBroken' : 'badCode')}</p>` : ''}<textarea class="code" id="codein" placeholder="${t('pasteCode')}"></textarea>
...
function compareRows(a, b){
  const row = (key, va, vb, fmt, goodUp) => { const d = vb - va, cls = Math.abs(d) < 1e-6 ? 'muted' : (d > 0) === goodUp ? 'good' : 'bad', arr = Math.abs(d) < 1e-6 ? '→' : d > 0 ? '↑' : '↓';
    return `<div class="stat"><span>${ic(GLOSS[key].icon)} ${L2(GLOSS[key]).name}</span><b>${fmt(va)} <span class="${cls}">${arr} ${fmt(vb)}</span></b></div>`; };
...
function review(){
  ...
  D.projects.forEach(id => items.push(PN(id) + ': ' + L2(PROJ_TXT[id])[0] + ' (' + t(D.projMode[id] === 'fast' ? 'buildFast' : 'buildTender') + ')'));
  D.facilities.forEach(id => items.push(L2(FAC_TXT[id])[0]));
  if (D.wageRaise) items.push(`${ic('worker')} ${t('raiseTitle')} +${D.wageRaise}%`);
  ...
  <div class="row" style="margin-top:18px"><button class="btn primary" data-act="commit">${t('endSeason')} ${AR() ? 'play' : 'play'}</button><button class="btn" data-act="close">${t('goBack')}</button></div>`);
}
function commit(){
...
function showResults(before, chs){
  ...
  const mood = b > 1.5 ? ['faceOk', t('better')] : b < -1.5 ? ['faceSad', t('worse')] : ['faceFlat', t('same')];
  ...
  <h3 class="bh">${ic('link')} ${t('whyTitle')}</h3>${renderWhy(chs)}
...
function showEvent(){
  ...
  modal(`<div class="evhead"><div class="tut-icon" aria-hidden="true">${ic(EV_ICON[e.id] || 'siren', 'big')}</div><div><div class="src">${t('crisis')} · ${esc(EV_SRC[LANG][e.id])}</div><h2>${esc(tx[0])}</h2></div></div><p class="lede">${esc(tx[1])}</p>
  ...
  ${e.opts.map((o, i) => { const ok = optionAllowed(S, o); const why = ok.ok ? '' : (ok.why.includes('reserves') ? fill(t('needsUsd'), [-o.eff.usd]) : ok.why.includes('political') ? fill(t('needsInfluence'), [-o.eff.pc]) : (AR() ? 'غير متاح' : ok.why));
    return `<button class="opt" data-act="choose" data-i="${i}" ${ok.ok ? '' : 'disabled'}><b>${esc(tx[2][i][0])}</b><span class="t">${esc(tx[2][i][1])}${why ? ' (' + esc(why) + ')' : ''}</span><span class="chips">${effChips(o.eff)}</span></button>`; }).join('')}`, 'crisis');
}
function choose(i){
...
function histCard(id){ const h = L2(HIST_TXT[id]); return `<div class="histcard"><div class="src">${ic('book')} ${t('history')}</div><h4>${esc(h[0])}</h4><p>${esc(h[1])}</p></div>`; }
function showFail(id){
  ...
  modal(`<div class="tut-icon" aria-hidden="true">${ic('alert')}</div><h2>${esc(f[0])}</h2><div class="src">${esc(whenTxt(tn))} · ${fill(t('afterN'), [tn - (S.mission ? S.mission.start - 1 : 0)])}</div><p class="lede">${esc(fill(f[1], [revolts]))}</p>
...
function showLegacy(){
  ...
  <div class="row">${S.over && S.over.chapter ? `<button class="btn primary" data-act="nextChapter">${fill(t('chapterGo'), [(S.chapter || 1) + 1])} ${ic('play')}</button>` : ''}<button class="btn" data-act="restart">${t('playAgain')}</button><button class="btn" data-act="close">${t('lookMap')}</button></div>`);
}
