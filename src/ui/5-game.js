...
const EFX_ICON = { poor:'worker' };
function withEffects(title, fn){
  const before = clone(S), pB = step(before, 0.5);
  const ok = fn();
  if (!ok){ toast(' ' + t('cantNow')); return false; }   // a click that does nothing is a bug, not a no-op
...
function effChip([k, , good, txt]){ const g = GLOSS[k]; return `<span class="chip ${good ? 'up' : 'down'}">${EFX_ICON[k] || (g ? g.icon : 'star')} ${esc(txt)}</span>`; }
function renderEffect(){
  const el = $('#effectbox'); if (!el) return;
  const e = UI.effect; if (!e){ el.innerHTML = ''; return; }
  const A = AR();
  el.innerHTML = `<div class="effect" role="status"><button class="close" data-act="closeEffect" aria-label="${t('close')}">${ic('close')}</button><div class="et">${ic('check')} ${esc(e.title)}</div>
...
function provBadge(id){ const pv = S.provs[id]; return (pv.project === true ? 'check' : pv.project === 'building' ? 'crane' : '') + (tierOf(pv.u) === 'revolt' ? 'alert' : ''); }
function personas(s){
...
function whyLive(){
  ...
  if (Math.abs(pct) >= 1){
    ...
    out.push({ icon:'exchange', steps:ch, weight:Math.abs(pct) * 2 });
  }
  if ((S.clogged || 0) > 3) out.push({ icon:'ship', weight:6, steps:[st(W.clogged, 'cause'), st(fill(W.lostExports, [usdM(S.clogged * 2)]), 'bad')] });
  { const dt = c.trust - a.trust; if (Math.abs(dt) >= 0.5){
      ...
      if (parts.length) out.push({ icon:'handshake', weight:Math.abs(dt) * 1.5, steps:[st(parts.map(([k, v]) => `${W['t_' + k]} (${sign(v, 0)})`).join(' + '), 'cause'), st(dt < 0 ? fill(W.trustDown, [Math.abs(dt).toFixed(1)]) : fill(W.trustUp, [dt.toFixed(1)]), dt < 0 ? 'bad' : 'good')] });
  } }
  { const da = c.anger - a.anger; if (Math.abs(da) >= 0.5){
      ...
      if (parts.length) out.push({ icon:'flame', weight:Math.abs(da) * 1.5, steps:[st(parts.map(([k]) => W[(da > 0 ? 'a_' : 'b_') + k]).join(' + '), 'cause'), st(da > 0 ? fill(W.angerUp, [da.toFixed(1)]) : fill(W.angerDown, [Math.abs(da).toFixed(1)]), da > 0 ? 'bad' : 'good')] });
  } }
  { const d = c.cash - a.cash; if (Math.abs(d) >= 3){ const rows = S.last.ledger.syp.slice().sort((x, y) => d < 0 ? x[1] - y[1] : y[1] - x[1]); if (rows.length)
      out.push({ icon:'cash', weight:Math.abs(d) / 3, steps:[st(fill(d < 0 ? W.biggestCost : W.biggestIncome, [LEDGER[LANG][rows[0][0]] || rows[0][0]]), 'cause'), st(fill(d < 0 ? W.cashDown : W.cashUp, [Math.abs(d).toFixed(1)]), d < 0 ? 'bad' : 'good')] }); } }
  { const dh = c.power - a.power; if (dh >= 0.3) out.push({ icon:'bolt', weight:4, steps:[st(W.gridArrived, 'cause'), st(fill(W.powerUp, [dh.toFixed(1)]), 'good')] }); }
...
function renderHUD(P){
  ...
  const lvSub = plan ? fill(t('lvlBuilding'), [monthsTxt(plan.due - S.t)])
  ...
  return `<button class="turn lvl" data-act="levels" aria-label="${fill(t('levelN'), [lv])}">${ring}<div>
      <div class="yr">${fill(t('levelN'), [lv])}${plan || lvReady ? `<span class="lvdot" aria-hidden="true">${ic(plan ? 'crane' : 'star')}</span>` : ''}</div>
      <div class="ss lvgoal">${esc(lvSub)}</div>
      <div class="mbarwrap lvlbar${plan ? ' building' : ''}"><i style="width:${toNext.toFixed(0)}%"></i></div></div></button>
    <button class="scorebadge g-${g}" data-act="showScore" aria-label="${t('score')}: ${Math.round(sc)}"><span class="sg">${g}</span><span><span class="sn">${Math.round(sc)}</span><span class="sl">${S.chapter > 1 ? fill(t('chapterN'), [S.chapter]) + ' · ' : ''}${esc(MONTHS[LANG][monthOf(S)])} ${yearNow(S)} <span class="dl ${dsc > 0.05 ? 'up' : dsc < -0.05 ? 'down' : 'flat'}">${dsc > 0.05 ? 'up' : dsc < -0.05 ? 'down' : '•'} ${sign(dsc, 1)}</span></span></span>${pop}</button>
    ...
    <div class="hudbtns">
    <button class="iconbtn" data-act="mute" aria-label="${SFX.on ? t('soundOff') : t('soundOn')}" title="${SFX.on ? t('soundOff') : t('soundOn')}">${ic(SFX.on ? 'soundOn' : 'soundOff')}</button>
    <button class="iconbtn lang" data-act="lang" aria-label="${t('language')}">${ic('globe')} <span class="lt2">${t('language')}</span></button>
    <button class="iconbtn" data-act="menu" aria-label="${t('menu')}">${ic('menu')}</button><!--hudbtns--></div>`;
}
...
const ADVISORS = [['econ', 'worker', 'economist'], ['sec', 'trophy', 'securityChief'], ['health', 'clinic', 'ministerHealth'], ['ind', 'factory', 'ministerInd']];
...
function renderAdvisors(P){
  ...
  const por = ([k, icon, name]) => `<button class="portrait" data-act="adv" data-v="${k}" aria-pressed="${UI.advOpen && cur === k}" aria-label="${esc(t(name))}" title="${esc(t(name))}">${ic(icon)}<span class="dot ${a[k].lvl}"></span></button>`;
  return `<div class="advisors"><div class="portraits">${shown.map(por).join('')}</div>
    ${UI.advOpen ? `<div class="bubble"><button class="advhide" data-act="advhide" aria-label="${t('hide')}" title="${t('hide')}">${ic('close')}</button><div class="who">${esc(t(who[2]))}</div>${esc(x.text)}<span class="act">${esc(x.act)}</span>
...
function renderProvince(){
  ...
  const A = AR(), good = [];
  if (x.unrest) good.push(`${ic('flame')} ${sign(x.unrest)}`); if (x.power) good.push(`${ic('bulb')} +${x.power}${A ? 'س' : 'h'}`); if (x.jobs) good.push(A ? `${ic('briefcase')} ${-x.jobs} فرصة عمل` : `${ic('briefcase')} ${-x.jobs} jobs`);
  if (x.rev) good.push(`${ic('cash')} +${bn(x.rev * 2)}/${A ? 'سنة' : 'yr'}`); if (x.transit) good.push(`${ic('bank')} +${usdM(x.transit * 2)}/${A ? 'سنة' : 'yr'}`); if (x.phosphate) good.push(`${ic('pickaxe')} +${usdM(x.phosphate * 2)}/${A ? 'سنة' : 'yr'}`); if (x.oil) good.push(`${ic('barrel')} +${usdM(x.oil * 2)}/${A ? 'سنة' : 'yr'}`);
  if (x.wheat) good.push(`${ic('wheat')} ${MINUS}${usdM(x.wheat * 2)}/${A ? 'سنة' : 'yr'}`); if (x.mw) good.push(`${ic('bolt')} +${x.mw} MW`); if (x.cap) good.push(A ? ' اقتصاد أكبر' : ' bigger economy'); if (x.trust) good.push(`${ic('handshake')} +${x.trust}`);
  ...
  else if (pv.project === 'building') action = `<span class="chip">${ic('crane')} ${fill(t('building'), [monthsTxt(pending ? pending.due - S.t : 1)])}</span>`;
  else action = `<div class="contract">
      <button class="opt mini" data-act="proj" data-id="${id}" data-mode="fast" ${why ? 'disabled' : ''}><b>${ic('bolt')} ${t('buildFast')}</b><span class="t">${fill(t('fastTxt'), [monthsTxt(mF), lF])}</span></button>
      <button class="opt mini" data-act="proj" data-id="${id}" data-mode="tender" ${why ? 'disabled' : ''}><b>${ic('scales')} ${t('buildTender')}</b><span class="t">${fill(t('tenderTxt'), [monthsTxt(mT), lT])}</span></button></div>`;
  const note = id === 'rif' ? t('rifNote') : id === 'damascus' ? t('damNote') : '';
  return `<aside class="pcard"><div class="head"><div><h2>${esc(PN(id))}</h2><span class="tierpill" style="background:${TIER_COL[tier]}">${tierName(tier)}</span></div><button class="close" data-act="closeProv" aria-label="${t('close')}">${ic('close')}</button></div>
    <div class="body"><p class="muted" style="margin:0 0 10px;font-size:13px">${note}${fill(t('people'), [p.pop.toFixed(1)])}</p>
      ...
      <div class="quest${pv.project === true ? ' done' : ''}"><div class="qt">${ic('crane')} ${t('bigProject')}</div><h4>${esc(tx[0])}</h4><p><b>${t('problem')}</b> ${esc(tx[1])}</p>
...
function renderDecrees(){
  return DECREES.map(x => {
    ...
    return `<div class="dcard${done ? ' done' : ''}"><span class="gem">${ic('star')} ${x.pc}</span><h4>${esc(tx[0])}</h4><p class="kid">${esc(tx[1])}</p><div class="fx">${esc(tx[2])}</div>
...
function renderMoneyActions(){
  ...
  h += `<div class="group"><h3>${ic('star')} ${t('buyTitle')}</h3><p>${t('buyText')}</p>
    <div class="dcard"><span class="gem">${ic('star')} +8</span><h4>${t('giftTitle')}</h4><div class="row spread"><span class="chip">${ic('cash')} ${bn(7)}</span><button class="btn primary" data-act="grantPop" ${giftCd ? 'disabled' : ''}>${t('choose')}</button></div>${giftCd ? `<div class="why">${cdTxt('lastGift')}</div>` : ''}</div>
    <div class="dcard"><span class="gem">${ic('star')} +6</span><h4>${t('reliefTitle')}</h4><div class="row spread"><div class="row"><span class="chip">${ic('bank')} ${usdM(40)}</span><span class="chip up">${fill(CHIP[LANG].trust, ['+2'])}</span></div><button class="btn primary" data-act="relief" ${relCd || S.reserves < 40 ? 'disabled' : ''}>${t('choose')}</button></div>${relCd ? `<div class="why">${cdTxt('lastRelief')}</div>` : ''}</div></div>`;
  {
    ...
    h += `<div class="group"><h3>${ic('compass')} ${t('repayTitle')}</h3><p>${fill(t('repayText'), [usdM(S.debt), usdM(SOV_PER_USD)])}</p>
  ...
  h += `<div class="group"><h3>${ic('globe')} ${t('abroadTitle')}</h3><p>${t('abroadText')}${S.grant > 0 ? ' ' + fill(t('grantOnHand'), [usdM(S.grant)]) : ''}</p>` + FACILITIES.map(f => {
    ...
    return `<div class="dcard${st ? ' done' : ''}">${f.pc ? `<span class="gem">${ic('star')} ${f.pc}</span>` : ''}<h4>${esc(tx[0])}</h4><p class="kid">${ic('bank')} <b>${usdM(total)}</b> · ${esc(tx[1])}</p>
...
function renderTradeResources(){
  ...
  let h = `<div class="rcard oil"><div class="rh"><span class="ri">${ic('barrel')}</span><div><h3>${t('oilTitle')}</h3><div class="rv">${fill(t('oilProd'), [o.prod.toFixed(0)])}</div></div></div>
    ...
    <div class="split"><span>${ic('house')} ${fill(t('kbbl'), [o.home.toFixed(0)])}</span><div class="splitbar"><i style="width:${o.prod ? o.home / o.prod * 100 : 0}%"></i></div><span>${ic('ship')} ${fill(t('kbbl'), [o.exp.toFixed(0)])}</span></div>
    <p class="small">${fill(t('oilUseHint'), [o.refineCap])}</p></div>`;
  h += `<div class="rgrid">
    <div class="rcard"><div class="rh"><span class="ri">${ic('flame')}</span><div><h3>${t('gasTitle')}</h3><div class="rv">${fill(t('gasProd'), [r.gas.toFixed(0)])}</div></div></div>${bar(r.gas, 30, '#35b6a3')}<p class="small">${t('gasHint')}</p></div>
    <div class="rcard"><div class="rh"><span class="ri">${ic('pickaxe')}</span><div><h3>${t('phosTitle')}</h3><div class="rv">×${r.phos.toFixed(2)}</div></div></div>${bar(r.phos, 2.1, '#8c5cc7')}<p class="small">${t('phosHint')}</p></div>
    <div class="rcard"><div class="rh"><span class="ri">${ic('olive')}</span><div><h3>${t('farmTitle')}</h3><div class="rv">${ic('dotFill').repeat(r.farm)}${ic('dot').repeat(Math.max(0, 4 - r.farm))}</div></div></div><p class="small">${t('farmHint')}</p></div></div>`;
  ...
  const investCard = id => {
    ...
    const status = running ? `<span class="chip">${ic('hourglass')} ${fill(t('running'), [monthsTxt(running.due - S.t)])}</span>` : maxed ? `<span class="chip up">${id === 'offshore' ? (r.offshore === 'found' ? ' ' + (A ? 'وُجد غاز' : 'Gas found') : ' ' + (A ? 'بئر جافة' : 'Dry well')) : t('maxed')}</span>`
      : `<button class="btn primary" data-act="invest" data-id="${id}" ${why ? 'disabled' : ''}>${t('investBtn')}</button>`;
    return `<div class="dcard inv"><span class="gem">${ic('bank')} ${usdM(cost)}</span><h4>${INV_TXT[id].icon} ${esc(tx[0])}</h4><p class="kid">${esc(tx[1])}</p>
      <div class="row spread"><div class="row"><span class="chip">${ic('hourglass')} ${monthsTxt(x.months)}</span>${x.jobs ? `<span class="chip up">${ic('briefcase')} ${fill(t('jobsChip'), ['+' + x.jobs])}</span>` : ''}${x.gamble ? `<span class="chip down">${ic('dice')} ${t('gamble')}</span>` : ''}${pb ? `<span class="chip up">${fill(t('payback'), [monthsTxt(Math.round(pb * 12 / 6) * 6)])}</span>` : `<span class="chip">${t('paybackNever')}</span>`}${built}</div>${status}</div>
  ...
  if (SECTORS.length) h += `<h3 class="bh">${ic('factory')} ${t('sectorTitle')}</h3><p class="small muted">${t('sectorSub')}</p>` + SECTORS.map(investCard).join('');
  h += `<h3 class="bh">${ic('pickaxe')} ${t('extractTitle')}</h3><p class="small muted">${t('extractSub')}</p>` + DIG.map(investCard).join('');
  return h;
}
function renderTradePorts(){
  const want = (S.exportWant || 0) * 2, cap = (S.exportCap || exportCapacity(S)) * 2, A = AR();
  let h = `<div class="rcard"><div class="rh"><span class="ri">${ic('box')}</span><div><h3>${t('portsTitle')}</h3><div class="small muted">${t('portsSub')}</div></div></div>
    <div class="capmeter"><i class="${want > cap ? 'over' : ''}" style="width:${clamp(want / Math.max(cap, 1) * 100, 0, 100)}%"></i></div>
    <p class="small">${fill(t('portsUse'), [usdM(want), usdM(cap)])}</p>${S.clogged > 1 ? `<p class="why">${ic('alert')} ${fill(t('clogged'), [usdM(S.clogged * 2)])}</p>` : ''}</div>`;
  h += ['latakia', 'tartus'].map(id => {
    const p = S.ports[id], up = S.pipe.find(x => x.kind === 'port' && x.id === id);
    return `<div class="dcard port"><h4>${ic('anchor')} ${PORT_NAME[LANG][id]}</h4><div class="stars">${Array.from({ length:Math.max(3, p.lvl) }, (_, i) => `<span class="${i < p.lvl ? 'on' : ''}">${ic('anchor')}</span>`).join('')} <span class="small muted">${fill(t('portLvl'), [p.lvl])} · ${p.op === 'foreign' ? t('portForeign') : t('portState')}</span></div>
      ${up ? `<span class="chip">${ic('hourglass')} ${fill(t('running'), [monthsTxt(up.due - S.t)])}</span>` :
      `<div class="contract"><button class="opt mini" data-act="portUp" data-id="${id}" ${S.reserves < portCost(p.lvl) || p.lvl >= invCap(S) ? 'disabled' : ''}><b>${ic('crane')} ${t('upgrade')}</b><span class="t">${fill(t('upgradeTxt'), [usdM(portCost(p.lvl)), monthsTxt(PORT_UPGRADE.months)])}</span>${p.lvl >= invCap(S) ? `<span class="t">${fill(t('lvlCapped'), [invCap(S)])}</span>` : S.reserves < portCost(p.lvl) ? `<span class="t">${fill(t('needsUsd'), [portCost(p.lvl)])}</span>` : ''}</button>
        ${p.op === 'state' ? `<button class="opt mini" data-act="portCon" data-id="${id}"><b>${ic('handshake')} ${t('concession')}</b><span class="t">${t('concessionTxt')}</span></button>` : ''}</div>`}</div>`;
  }).join('');
  return h;
}
...
function renderTradeFirms(){
  ...
  h += FIRM_IDS.map(id => {
    ...
    const body = got
      ? `<span class="chip up">${ic('check')} ${fill(t('firmIn'), [esc(L2(INV_TXT[got.sector])[0])])}</span>
         ${building ? `<span class="chip">${ic('hourglass')} ${fill(t('running'), [monthsTxt(building.due - S.t)])}</span>` : `<span class="chip down">${fill(t('firmTakes'), [usdM(cut(got.sector))])}</span>`}`
    ...
    return `<div class="dcard partner${got ? ' on' : ''}"><div class="ph"><span class="flag" aria-hidden="true">${ic(f.flag)}</span>
...
function renderTradePartners(){
  return `<p class="small muted" style="margin-top:0">${t('partnersSub')}</p>` + Object.keys(PARTNERS).map(id => {
    ...
    const status = d ? (d.on ? `<span class="chip up">${ic('check')} ${t('dealActive')}</span>` : `<span class="chip down">${ic('pause')} ${t('dealPaused')}</span>`) : `<button class="btn primary" data-act="deal" data-id="${id}" ${why ? 'disabled' : ''}>${t('dealSign')}</button>`;
    return `<div class="dcard partner${d ? (d.on ? ' on' : ' paused') : ''}"><span class="gem">${ic('star')} ${x.pc}</span><div class="ph"><span class="flag" aria-hidden="true">${ic(x.flag)}</span><div><div class="small muted">${esc(tx[0])}</div><h4>${esc(tx[1])}</h4></div></div>
      <p class="kid">${esc(tx[2])}</p><div class="fx">${ic('star')} ${esc(tx[3])}</div><div class="small muted">${ic('lock')} ${t('needs')} ${esc(tx[4])}${x.sov ? ` · ${t('independence')} ${MINUS}${x.sov}` : ''}</div>
...
const GUIDE_TASKS = [
  { id:'start',   icon:'play' },
  { id:'gloss',   icon:'search' },
  { id:'policy',  icon:'scroll', go:'policy' },
  { id:'prov',    icon:'map' },
  { id:'project', icon:'crane' },
  { id:'money',   icon:'cash', go:'money' },
];
...
function renderGuide(){
  ...
  // is working towards and how far off it is. It used to sit under six other things, off-screen.
  {
    ...
    h += `<h3 class="bh" style="margin-top:0">${ic('star')} ${fill(t('lvlReached'), [lv])} — ${esc(levelName(lv))}
      <button class="btn small" data-act="levels">${t('lvlLadder')} →</button></h3>`;
    if (plan){
      h += `<div class="quest done"><div class="qt">${ic('crane')} ${t('lvlTargets')}</div>
      ...
      h += `<div class="row" style="margin-bottom:8px"><span class="chip${met === nd.length ? ' up' : ''}">${fill(t('lvlProgress'), [met, nd.length])}</span>
        <span class="chip">${ic('factory')} ${fill(t('lvlCapNow'), [invCap(S)])}</span></div>`;
      h += needRows(S);
    }
  }
  ...
  if (left){
    ...
    h += `<h3 class="bh">${ic('compass')} ${t('guideSteps')} <span class="chip">${fill(t('guideLeft'), [left])}</span></h3>`;
    h += GUIDE_TASKS.map(x => {
      const ok = guideDone(x.id), isNext = !ok && x.id === next.id;
      return `<div class="gstep${ok ? ' done' : ''}${isNext ? ' now' : ''}">
        <span class="gmark" aria-hidden="true">${ic(ok ? 'check' : isNext ? x.icon : 'dot')}</span>
    ...
    h += `<p class="mpnote">${ic('check')} ${t('guideDone')}</p>`;
  }
  ...
  h += `<h3 class="bh">${ic('chevron')} ${t('guideNext')}</h3>
  ...
  const chs = whyLive();
  h += `<h3 class="bh">${ic('link')} ${t('guideWhy')}</h3>` + (chs.length ? renderWhy(chs) : `<p class="muted small">${t('guideNothing')}</p>`);

  // and the chains themselves, always available
  h += `<h3 class="bh">${ic('gear')} ${t('guideChains')}</h3><ul class="chainlist">`
...
const SVC_ICON = { schools:'school', clinics:'clinic', unis:'graduation' };
function renderServices(){
  const meterRow = (key, name, v) => `<div class="rcard"><div class="rh"><span class="ri" aria-hidden="true">${ic(GLOSS[key].icon)}</span><div><h3>${name}</h3><div class="rv">${Math.round(v)} / 100</div></div></div>
  ...
  h += Object.keys(SERVICES).filter(id => id !== 'unis' || isOpen('unis')).map(id => {
    ...
    return `<div class="dcard"><span class="gem">${ic('bank')} ${usdM(x.usd)}</span><h4>${ic(SVC_ICON[id])} ${esc(svcLabel(id))}</h4><p class="kid">${esc(txt)}</p>
      <div class="bar big"><i style="width:${clamp(cov * 100, 0, 100)}%;background:${enough ? 'var(--good)' : 'var(--tense)'}"></i></div>
      <div class="row spread" style="margin-top:8px"><div class="row"><span class="chip${enough ? ' up' : ''}">${fill(t('svcCover'), [have, need])}</span><span class="chip">${ic('cash')} ${bn(x.syp)}</span><span class="chip">${ic('hourglass')} ${monthsTxt(x.months)}</span>${enough ? `<span class="chip up">${t('svcEnough')}</span>` : `<span class="chip down">${t('svcNeedMore')}</span>`}</div>
      ${running ? `<span class="chip">${ic('hourglass')} ${fill(t('svcRunning'), [monthsTxt(running.due - S.t)])}</span>` : `<button class="btn primary" data-act="svc" data-id="${id}" ${why ? 'disabled' : ''}>${t('svcBuild')}</button>`}</div>
      ${why ? `<div class="why">${esc(why)}</div>` : ''}</div>`;
  }).join('');
  return h;
}
function renderPopulation(){
  ...
  return `<div class="rcard"><div class="rh"><span class="ri" aria-hidden="true">${ic('people')}</span><div><h3>${t('popTitle')}</h3><div class="rv">${fill(t('popNow'), [S.popM.toFixed(1)])}</div></div></div>
...
function renderBarCard(){
  ...
  return `<div class="rcard"><div class="rh"><span class="ri" aria-hidden="true">${ic('chart')}</span><div><h3>${t('barTitle')}</h3><div class="rv">${fill(t('barNow'), [pct])}</div></div></div>
...
function renderWhyPanel(){
  const chs = whyLive();
  return renderBarCard() + `<h3 class="bh">${ic('link')} ${t('whyNow')}</h3>` + renderWhy(chs);
}
...
function renderDrawer(){
  ...
  return `<aside class="drawer"><div class="head"><span class="dic" aria-hidden="true">${ic(d[1])}</span><h2>${t(d[2])}</h2><button class="close" data-act="closeDrawer" aria-label="${t('close')}">${ic('close')}</button></div>
...
function pipeLabel(i){
  if (i.kind === 'proj') return ic('crane') + ' ' + esc(PN(i.id));
  if (i.kind === 'port') return ic('anchor') + ' ' + esc((PORT_NAME[LANG] || {})[i.id] || i.id);
  if (i.kind === 'svc') return ic(SVC_ICON[i.id] || 'school') + ' ' + esc(svcLabel(i.id) || i.id);
  if (i.kind === 'lvl') return ic('star') + ' ' + esc(fill(t('lvlReached'), [i.to]));
  if (i.kind === 'firm'){
    const f = FIRM_TXT[i.id], sec = INV_TXT[i.sector];
    return ic('building') + ' ' + esc((f ? L2(f)[1] : i.id) + (sec ? ' — ' + L2(sec)[0] : ''));
  }
  const x = INV_TXT[i.id];
  return x ? ic(x.icon) + ' ' + esc(L2(x)[0]) : ic('factory') + ' ' + esc(String(i.id || ''));
}
function renderProgressCharts(){
  ...
  let h = `<h3 class="bh" style="margin-top:0">${ic('hourglass')} ${t('comingSoon')}</h3>`;
  if (mwSoon > 1) h += `<div class="pipe"><span>${ic('bolt')} ${fill(t('mwArrives'), [Math.round(mwSoon)])}</span><b>${t('inTime').replace('{0}', monthsTxt(12))}</b></div>`;
  ...
  h += `<h3 class="bh">${ic('chart')} ${t('subCharts')}</h3>` + (S.history.length < 3 ? `<p class="muted" style="font-size:13px">${t('chartsEmpty')}</p>` :
...
function renderDock(){
  ...
  const sp = [[0, 'pause', 'pause'], [1, 'play', 'slow'], [2, 'fast', 'normal'], [3, 'faster', 'fastest']];
  ...
  return `<div class="dgroup">${rest.slice(0, 3).map(btn).join('')}</div>${tr ? `<div class="dgroup trade">${btn(tr)}</div>` : ''}${rest.length > 3 ? `<div class="dgroup">${rest.slice(3).map(btn).join('')}</div>` : ''}
    ...
    <button class="influence" data-act="drawer" data-v="decrees"><span class="st" aria-hidden="true">${ic('star')}</span><span><span class="n">${Math.round(S.pc)}</span><span class="l">${t('influenceLbl')}</span></span></button>
    ${S.over ? `<button class="endturn" data-act="restart">${t('playAgain')}</button>`
      : S.live ? `<div class="liveclock" role="status"><span class="lc" aria-hidden="true">${ic('hourglass')}</span><span><span class="n" id="liveclock">${liveLeftTxt()}</span><span class="l">${t('nextMonth')}</span></span></div>`
      : `<div class="clock" role="group" aria-label="${t('play')}">${sp.map(([v, icn, l]) => `<button class="cbtn${v === 0 ? ' pause' : ''}" data-act="speed" data-v="${v}" aria-pressed="${UI.speed === v}" aria-label="${t(l)}" title="${t(l)}"><span dir="ltr">${ic(icn)}</span></button>`).join('')}</div>`}`;
}
...
function render(force){
  ...
  $('#startcue').innerHTML = !S.over && UI.speed === 0 && S.t === (S.mission ? S.mission.start : 0) ? `<button class="startcue" data-act="speed" data-v="1">${ic('play')} ${t('startPrompt')}</button>` : '';
  if (force || !UI.hover){
...
function renderScorePanel(){
  ...
  return `<div class="row" style="align-items:center;gap:14px">
    ...
    <h3 class="bh">${ic('trophy')} ${t('scoreParts')}</h3>${scoresBlock()}
    <p class="muted" style="font-size:13px">${t('scoreAvg')}</p>
    <div class="dcard"><h4>${ic('compass')} ${esc(G.name)} — ${Math.round(S.sov)}</h4>
      <p class="kid">${esc(G.what)}</p>
      ${gp > 0.01 ? `<p class="warnbox">${ic('alert')} ${t('sovBite')}</p>` : ''}
...
UI.live = false;   // one clock. The live/all-at-once pick is gone; this stays false.
// One way in. No clock pick, no difficulty pick, no mission list: someone who has never run a
// country should not have to choose how hard it is before they know what it is. Learner IS the
// game now. The other modes still exist in the engine (MISSIONS, diff 'realistic', UI.live) and
// are one button each to put back — see docs/PROGRESS.md.
function startScreen(){
  modal(`<div class="row spread"><div class="tut-icon" aria-hidden="true">${ic('olive', 'big')}</div><button class="btn" data-act="lang">${ic('globe')} ${t('language')}</button></div>
  <h2>${t('title')}</h2><div class="src">${t('subtitle')}</div>
  <p class="lede">${t('howPlay')}</p>
  <button class="opt" data-act="newgame" data-v="learner"><b>${t('startBtn')}</b><span class="t">${t('startTxt')}</span></button>
...
function nextChapter(){
  ...
  toast(fill(t('chapterStart'), [S.chapter]), 'year'); sfx('cycle');
}
...
function showAway(r){
  ...
  const rows = [
    ['trophy', t('score'), a.score, c.score, 1, v => v.toFixed(0)],
    ['handshake', L2(GLOSS.trust).name, a.trust, c.trust, 1, v => v.toFixed(0)],
    ['flame', L2(GLOSS.anger).name, a.anger, c.anger, -1, v => v.toFixed(0)],
    ['bank', L2(GLOSS.usd).name, a.usd, c.usd, 1, usdM],
    ['bulb', L2(GLOSS.power).name, a.power, c.power, 1, v => v.toFixed(1) + (A ? 'س' : 'h')],
    ['briefcase', L2(GLOSS.jobs).name, a.jobs, c.jobs, -1, v => v.toFixed(0) + '%'],
  ...
  modal(`<div class="tut-icon" aria-hidden="true">${ic('sunrise')}</div><h2>${t('awayTitle')}</h2>
    <div class="src">${fill(t('awaySub'), [monthsTxt(r.months), esc(whenTxt(S.t))])}</div>
    <div class="scores" style="margin-top:14px">${rows.map(([icn, name, from, to, good, fmt]) => {
      ...
      return `<div><div class="row spread"><span>${ic(icn)} ${esc(name)}</span><b class="${well ? 'good' : 'bad'}">${rose ? '↑' : '↓'} ${esc(fmt(to))}</b></div>
        <div class="small muted" style="margin:0">${esc(fmt(from))} → ${esc(fmt(to))}</div></div>`; }).join('')}</div>
    ${done.length ? `<h3 class="bh">${ic('check')} ${t('awayDone')}</h3>${done.map(n => `<div class="pipe"><span>${esc(noteText([S.t, ...n]))}</span></div>`).join('')}` : ''}
    ${r.queued ? `<p class="tipbox">${ic('alert')} ${fill(t('awayWaiting'), [r.queued])}</p>` : ''}
    <div class="row"><button class="btn primary" data-act="awayGo">${r.queued ? t('awaySee') : t('keepPlaying')} ${ic('play')}</button></div>`);
}
...
function needRows(s){
  return levelNeeds(s).map(d => {
    ...
    return `<div class="gstep lvlstep${ok ? ' done' : ' now'}"><span class="gmark">${ic(ok ? 'check' : 'dot')}</span>
...
function showLevels(){
  ...
  let h = `<div class="tut-icon" aria-hidden="true">${ic(lv >= LEVEL_MAX ? 'olive' : 'star', 'big')}</div>
    <h2>${fill(t('levelN'), [lv])} — ${esc(levelName(lv))}</h2>
    <div class="src">${esc(levelBlurb(lv))}</div>`;
  if (plan){
    h += `<div class="quest done" style="margin-top:14px"><div class="qt">${ic('crane')} ${t('lvlTargets')}</div>
      <h4>${fill(t('lvlReached'), [plan.to])}</h4>
      <p>${esc(fill(t('lvlPlanNow'), [monthsTxt(plan.due - S.t)]))}</p></div>`;
  } else {
    h += `<h3 class="bh">${ic('target')} ${fill(t('lvlNextTargets'), [next])}</h3>
      <div class="row" style="margin-bottom:8px"><span class="chip${met === nd.length ? ' up' : ''}">${fill(t('lvlProgress'), [met, nd.length])}</span></div>
      ${needRows(S)}`;
  }
  if (gifts.length) h += `<h3 class="bh">${ic('unlock')} ${fill(t('lvlOpensAt'), [next])}</h3>
  ...
  h += `<h3 class="bh">${ic('ladder')} ${t('lvlLadder')}</h3><div class="ladder">`;
  for (let n = 1; n <= LEVEL_MAX; n++){
    const cls = n < lv ? 'done' : n === lv ? 'here' : '';
    h += `<div class="rung ${cls}"><b>${n}</b><span>${esc(levelName(n))}</span>${n === LEVEL_MAX ? '<i></i>' : n === lv ? '<i></i>' : n < lv ? '<i></i>' : ''}</div>`;
  }
  if (lv > LEVEL_MAX) h += `<div class="rung here"><b>${lv}</b><span>${esc(levelName(lv))}</span><i>${ic('pin')}</i></div>`;
  h += `</div>`;
...
function showLevelUp(n){
  ...
  modal(`<div class="tut-icon" aria-hidden="true">${ic('star')}</div><h2>${fill(t('lvlReached'), [n])}</h2>
    <div class="src">${esc(levelName(n))}</div>
    <p class="lede">${esc(levelBlurb(n))}</p>
    ${gifts.length ? `<h3 class="bh">${ic('unlock')} ${t('stageTitle')}</h3>
    ...
    <h3 class="bh">${ic('target')} ${fill(t('lvlNextTargets'), [n + 1])}</h3>
    ${needRows(S)}
...
function showSummit(){
  ...
  modal(`<div class="tut-icon" aria-hidden="true">${ic('olive')}</div><h2>${t('summitTitle')}</h2>
    <div class="src">${fill(t('summitSub'), [yearNow(S)])}</div>
    <p class="lede">${t('summitText')}</p>
    <div class="scores">
      <div class="row spread"><span>${ic('trophy')} ${t('score')}</span><b>${Math.round(Lg.avg)} ${Lg.grade}</b></div>
      <div class="row spread"><span>${ic('bulb')} ${L2(GLOSS.power).name}</span><b>${nationalHours(S).toFixed(1)}${A ? 'س' : 'h'}</b></div>
      <div class="row spread"><span>${ic('briefcase')} ${L2(GLOSS.jobs).name}</span><b>${Math.round(joblessNat(S))}%</b></div>
      <div class="row spread"><span>${ic('graduation')} ${t('eduName')}</span><b>${Math.round(S.edu)}</b></div>
      <div class="row spread"><span>${ic('crane')} ${t('builtLbl')}</span><b>${PROVS.filter(p => S.provs[p.id].project === true).length} / ${PROVS.length}</b></div>
      <div class="row spread"><span>${ic('people')} ${t('poorLbl')}</span><b>${Math.round(cl.poor)}%</b></div>
    </div>
...
function showMilestone(){
  ...
  modal(`<div class="row" style="align-items:flex-end;gap:18px"><div class="grade">${Lg.grade}</div><div><h2>${fill(t('milestoneTitle'), [yrs])}</h2><div class="src">${fill(t('milestoneSub'), [esc(whenTxt(S.t))])}</div></div></div>
    ${scoresBlock()}<h3 class="bh">${ic('link')} ${t('whyNow')}</h3>${renderWhy(chs)}
    <div class="row" style="margin-top:14px"><button class="btn primary" data-act="close">${t('keepPlaying')} ${ic('play')}</button></div>`);
}
function choose(i){
...
function showFail(id){
  ...
  modal(`<div class="tut-icon" aria-hidden="true">${ic('alert')}</div><h2>${esc(f[0])}</h2><div class="src">${esc(whenTxt(S.t))} · ${fill(t('afterN'), [monthsTxt(S.t - (S.mission ? S.mission.start : 0))])}</div><p class="lede">${esc(fill(f[1], [revolts]))}</p>
  <p class="tipbox">${ic('bulb')} <b>${t('tipLabel')}</b> ${esc(f[2])}</p>${histCard(hist)}${scoresBlock()}
...
function showMissionEnd(){
  ...
  modal(`<div class="tut-icon" aria-hidden="true">${ic(won ? 'trophy' : 'target', 'big')}</div><h2>${won ? t('missionWon') : t('missionLost')}</h2><div class="src">${MISSION_TXT[id].icon} ${esc(m[0])}</div>
  <p><b>${t('missionGoal')}:</b> ${esc(m[1])}</p><p class="tipbox">${ic('bulb')} ${esc(m[2])}</p>${histCard(hist)}
...
document.addEventListener('click', ev => {
  ...
  if (S && S.over && !always.includes(a)){ toast(' ' + t('overNow')); return; }   // never a dead click
  ...
  switch(a){
    ...
    case 'lang': { setLang(AR() ? 'en' : 'ar'); render(true); if ($('#modal [data-act=newgame]')) return startScreen(); if ($('#modal [data-menu]')) return menu(); return; }
...
(function(){
  ...
  if (restore()){
    ...
    if (UI.fromBak){ UI.fromBak = false; toast(t('restoredBak')); }
