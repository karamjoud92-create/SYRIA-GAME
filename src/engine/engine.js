...
const PROVS = [
  // id, name, popM, base unrest target, start unrest, damage $B, mines %, power mod, col,row, neighbors
  { id:'idlib', name:'Idlib', pop:3.0, base:30, u:48, dmg:8, mines:45, pmod:0, c:1, r:0, nb:['aleppo','hama','latakia'] },
  { id:'aleppo', name:'Aleppo', pop:4.2, base:30, u:50, dmg:25, mines:40, pmod:0, c:2, r:0, nb:['idlib','hama','raqqa'] },
  { id:'raqqa', name:'Raqqa', pop:0.8, base:34, u:52, dmg:7, mines:50, pmod:-1, c:3, r:0, nb:['aleppo','hasakeh','deir','hama','homs'] },
  { id:'hasakeh', name:'Hasakeh', pop:1.2, base:36, u:50, dmg:3, mines:20, pmod:-1, c:4, r:0, nb:['raqqa','deir'] },
  { id:'latakia', name:'Latakia', pop:1.2, base:32, u:46, dmg:2, mines:10, pmod:1, c:0, r:1, nb:['idlib','hama','tartus'] },
  { id:'hama', name:'Hama', pop:1.6, base:28, u:42, dmg:6, mines:30, pmod:0, c:1, r:1, nb:['idlib','aleppo','latakia','homs','raqqa','tartus'] },
  { id:'deir', name:'Deir ez-Zor', pop:1.0, base:38, u:56, dmg:9, mines:55, pmod:-1, c:3, r:1, nb:['raqqa','hasakeh','homs'] },
  { id:'tartus', name:'Tartus', pop:0.9, base:28, u:40, dmg:1, mines:5, pmod:1, c:0, r:2, nb:['latakia','hama','homs'] },
  { id:'homs', name:'Homs', pop:1.5, base:30, u:44, dmg:14, mines:35, pmod:0, c:2, r:1, nb:['hama','tartus','rif','deir','raqqa'] },
  { id:'damascus', name:'Damascus', pop:2.0, base:24, u:36, dmg:5, mines:5, pmod:2, c:1, r:2, nb:['rif','quneitra','daraa'] },
  { id:'rif', name:'Rural Damascus', pop:3.0, base:30, u:47, dmg:22, mines:30, pmod:0, c:2, r:2, nb:['damascus','homs','daraa','suwayda'] },
  { id:'quneitra', name:'Quneitra', pop:0.1, base:34, u:45, dmg:1, mines:60, pmod:0, c:0, r:3, nb:['damascus','daraa'] },
  { id:'daraa', name:'Daraa', pop:1.0, base:34, u:50, dmg:4, mines:35, pmod:0, c:1, r:3, nb:['quneitra','damascus','rif','suwayda'] },
  { id:'suwayda', name:'Suwayda', pop:0.4, base:40, u:58, dmg:1, mines:15, pmod:-1, c:2, r:3, nb:['daraa','rif'] },
];
...
const PROJECTS = {
  ...
  idlib:   { title:'Olive oil export co-op and orchard demining', issue:'Unexploded ordnance sits in the groves, and farmers sell oil at a loss to middlemen.', usd:20, syp:3, pc:5, rev:0.7, mines:-15, unrest:-12, cap:2, repair:0.25 },
  ...
  quneitra:{ title:'Returnee villages and mine clearance', issue:'The most mined land in the country, with the fewest people left to farm it.', usd:10, syp:1.5, pc:0, rev:0.1, mines:-25, unrest:-10, cap:0.5, repair:0.1 },
...
const DECREES = [
  ...
  { id:'demining', name:'National demining authority', desc:'Trained teams sweeping farmland province by province.', pc:15, syp:0, usd:0, fx:'Mines -4 points a turn everywhere. Costs $10M a turn.', ongoing:true },
];
...
const INVEST = {
  oilwells:{ usd:60, months:12, max:3, req:s => eastAnger(s) < 70 },
  refinery:{ usd:50, months:12, max:3 },
  gasfield:{ usd:80, months:18, max:3 },
  offshore:{ usd:150, months:36, gamble:true },
  phosphate:{ usd:50, months:12, max:3 },
  farm:{ usd:30, months:12, max:3 },
};
const PARTNERS = {
...
const PORT_UPGRADE = { usd:60, months:12 };
...
function newGame(seed, diff = 'learner', mission = null){
  ...
  const provs = {};
  PROVS.forEach(p => provs[p.id] = { id:p.id, u:p.u, dmg:p.dmg, dmg0:p.dmg, mines:p.mines, mod:0, power:0, project:false });
  return {
    v:5, diff, seed: seed ?? Math.floor(Math.random()*1e9), t:0,
...
function projMonths(id, mode){ return (clamp(Math.ceil(PROJECTS[id].usd / 30), 1, 3) + (mode === 'tender' ? 1 : 0)) * 6; }
...
function step(state, dt = MONTH, policyOverride){
  ...
  Object.entries(s.facilities).forEach(([id, st]) => {
    ...
    while (st.paid < f.tranches.length && endT >= st.signed + f.tranches[st.paid][0] * 6) {
      ...
      else if (f.grid) { s.pipe.push({ due:endT + 12, kind:'mw', mw:amt * 5 }); notes.push(['wbGrid', amt * 5]); }
  ...
  addS('running', -3 * Math.pow(pIdx, 0.7));
  ...
  let wheatCut = 0; ['hama','hasakeh','raqqa'].forEach(k => { if (built(s, k)) wheatCut += PROJECTS[k].wheat * (1 - projLeak(s, k)); });
  ...
  if (s.decrees.demining) addU('demining', -10);
  ...
  if (P.capex) s.pipe.push({ due:endT + 12, kind:'mw', mw:P.capex * 6 * gridEff * dt }); else s.mw -= 25 * dt;
  s.demand *= Math.pow(1.004, dt);
  ...
  s.expWage = 25 + Math.max(0, s.cap - 20) * 0.6;
  ...
  s.trust = clamp(s.trust + (s.trustTarget - s.trust) * relax(0.25, dt), 0, 100);
  ...
  s.corr = clamp(s.corr + (cT - s.corr) * relax(0.15, dt), 5, 100);
  ...
  s.comp = clamp(s.comp + (compT - s.comp) * relax(0.25, dt), 5, 95);
  ...
  // --- provinces ---
  const reconB = P.recon * dt / s.parallel;
  const privB = (s.trust > 40 ? (s.trust - 40) * s.cap * 0.0004 : 0) * dt;
  const totalDmg = PROVS.reduce((a, p) => a + s.provs[p.id].dmg, 0) || 1;
  const newU = {}, ap = { trust:0, pay:0, power:0, subsidies:0, security:0, damage:0, prices:0, local:0, neighbors:0 };
  ...
  PROVS.forEach(p => {
    ...
    const parts = { trust:(50 - s.trust) * 0.4, pay:clamp((s.expWage - rw) * 0.5, -10, 15), power:blackout * 0.8 - 10,
      ...
      damage:Math.min(12, pv.dmg / p.pop * 0.8) + pv.mines * 0.06, prices:(s.infl - 20) * 0.15, local:pv.mod };
    ...
    newU[p.id] = clamp(pv.u + (tgt - pv.u) * relax(0.3, dt) + contagion * dt, 0, 100);
    Object.keys(parts).forEach(k => ap[k] += parts[k] * p.pop); ap.neighbors += contagion * p.pop / 0.3; wsum += p.pop;
    const rep = Math.min(pv.dmg, (reconB + privB) * pv.dmg / totalDmg); pv.dmg -= rep; s.repaired += rep;
    pv.mines = Math.max(0, pv.mines - (0.5 + (s.decrees.demining ? 4 : 0)) * dt);
  });
...
function finishProject(s, item, notes){
  ...
  if (x.mines) pv.mines = Math.max(0, pv.mines + x.mines * k);
  if (x.mw) s.mw += x.mw * k;
...
const ACT = {
  ...
  facility(s, id){
    ...
    if (f.ring) s.grant += first; else if (f.grid) s.pipe.push({ due:s.t + 12, kind:'mw', mw:first * 5 }); else s.reserves += first;
  ...
  deal(s, id){
    ...
    if (id === 'china'){ ['latakia','tartus'].forEach(k => { if (s.ports[k].lvl < 3) s.pipe.push({ due:s.t + 12, kind:'port', id:k }); }); }
...
function legacy(s){
  const nu = natUnrest(s);
  const comp = {
    Stability: clamp(100 - nu, 0, 100),
    Livelihoods: clamp(realWage(s) / 150 * 100, 0, 100),
...
function drawEvent(s){
  if (s.t < 2 || rnd(s) > (s.diff === 'learner' ? 0.1 : 0.13)) return null;
...
if (typeof module !== 'undefined' && module.exports) module.exports = { startGame, MISSIONS, newGame, step, checkFail, legacy, natUnrest, realWage, nationalHours, drawEvent, EVENTS, applyEffects, optionAllowed, PROVS, tierOf, ACT, oilNumbers, exportCapacity, MONTH, yearNow, monthOf };
